    ...
    <div className="p-5 flex flex-col gap-3">
      <div
        ...
        className={`transition-all duration-700 overflow-hidden ease-in-out ${
          selectedVOD ? "max-h-249.75" : "max-h-0"
        }`}
      >
        {selectedVOD && (
          <iframe
            key={selectedVOD[0] || undefined}
            src={`https://player.twitch.tv/?video=${selectedVOD[0]}&time=${selectedVOD[1]}&autoplay=true&parent=${import.meta.env.VITE_TWITCH_PARENT_DOMAIN}`}
      ...
      {searchedMatchups.map((match) => (
        <button
          ...
          } p-3 justify-around flex items-center border-t rounded`}
          onClick={() => {
        ...
        >
          ...
          <div className="flex gap-2 text-center text-slate-300 ml-auto">
            ...
            <div className="min-w-3.5 text-red-400">{match.player?.deaths}</div>
            <span className="text-gray-five">/</span>
